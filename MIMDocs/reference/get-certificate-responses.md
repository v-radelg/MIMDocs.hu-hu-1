---
title: "Tanúsítvány választ kaphat |} Microsoft Docs"
description: 
keywords: 
author: msmbaldwin
ms.author: mbaldwin
manager: mbaldwin
ms.date: 10/17/2016
ms.topic: reference
ms.prod: identity-manager-2015
ms.service: microsoft-identity-manager
ms.technology: security
ms.assetid: b0559e7b-eaff-499b-8bcf-c6cdf5a89545
audience: developer
ms.reviewer: mwahl
ms.suite: ems
ms.openlocfilehash: 3ac905a25e41a6212efb5f59e2f47e4345382ec3
ms.sourcegitcommit: 02fb1274ae0dc11288f8bd9cd4799af144b8feae
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 07/13/2017
---
# <a name="get-certificate-responses"></a>Tanúsítvány választ kaphat
Elküldi az eszköztanúsítvány-kérelmeket, hogy a tanúsítvány (CA) a feldolgozás készlete.

**Megjegyzés:**: tanúsítvány válaszok száma túlléphetik a kérelmek számát jelenti, mert néhány profilsablonok konfigurálhatók úgy, hogy a kiszolgálón kulcsok/kérelmek előállítására vagy külső tanúsítványok.

**Megjegyzés:**: Ebben a témakörben látható URL-címek vannak viszonyítva; API telepítésekor kiválasztott állomásnév például: `https://api.contoso.com`.
##<a name="request"></a>Kérelem


Módszer  |Kérelem URL-címe  
---------|---------
POST     |/CertificateManagement/API/V1.0/Requests/{ID}/certificatedata

###<a name="url-parameters"></a>URL-cím Paraméterek
Paraméter | Leírás
--------|--------------
Azonosítója| Kötelező. A profilsablon, amelyet a házirend kibontani a megfelelő GUID azonosító.

###<a name="request-headers"></a>Kérelem fejlécei
Közös kérelemfejléc, lásd: [HTTP-kérelmek és válaszfejlécek](certificate-management-rest-api-service-details.md#http-request-and-response-headers) a *Tanúsítványkezelő REST API szolgáltatás részletei*.
###<a name="request-body"></a>Kérés törzsében
A kérelem törzsében az alábbi tulajdonságait tartalmazza.

Tulajdonság | Leírás
---------|-----------
pfxpassword | A jelszó is védelmet nyújtanak, amelyek a hívás által visszaadott eredmények személyes információcseréhez kapcsolódó (PFX).
certificaterequests | Az ügyfél által generált tanúsítványkérelmek listája. Ezek a kérelmek kell jöttek létre az a CertificateRequestGenerationOptions objektumban megadott paraméterek További információkért lásd: [Get kérelem generációja tanúsítványbeállításai](get-certificate-request-generation-options.md).
Megjegyzések | Lesz további megjegyzéseket tartalmazó karakterláncot az előzmények a művelet rögzíti.


##<a name="response"></a>Válasz
###<a name="response-codes"></a>Válaszkódok
Kód  |Leírás  
---------|---------
200     | OK
403 | Tiltott
500 | Belső hiba

###<a name="response-headers"></a>Válaszfejlécek
Közös válaszfejlécek, lásd: [HTTP-kérelmek és válaszfejlécek](certificate-management-rest-api-service-details.md#http-request-and-response-headers) a *Tanúsítványkezelő REST API szolgáltatás részletei*.
###<a name="response-body"></a>Választörzs
A sikeres, a JSON-szerializált listát ad vissza [Microsoft.Clm.Shared.Certificates.Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx) objektumok a következő tulajdonságokkal:

Tulajdonság| Leírás
------|-------------
CertificateId | A tanúsítvány azonosítóját.
Hiba | Amely a tanúsítványkérelem feldolgozása során történt hiba.
Jelzők | A tanúsítvány leíró jelzők.
IsExternal | Egy logikai érték, amely azt jelzi, hogy a tanúsítvány olyan külső tanúsítványt.
isKeyHistory | Egy logikai érték, amely azt jelzi, hogy a tanúsítvány egy kulcs előzmények tanúsítványt.
isPfx | Egy logikai érték, amely azt jelzi, hogy a tanúsítványkérelemre PFX formátumban.
isPkcs7 | Egy logikai érték, amely azt jelzi, hogy a tanúsítványkérelemre PKCS #7 formátumban.
isServerGenerated | Logikai érték, amely jelzi, ha a tanúsítvány a kiszolgáló generálta.
PFX | A PFX-tanúsítvány blobja.
Pkcs7 | A PKCS #7 tanúsítványblob.
TemplateCommonName | A tanúsítványsablon közös-nevet a tanúsítványt.
UseLocalMachineStore | Egy logikai érték, amely azt jelzi, ha a tanúsítványt a helyi számítógép tanúsítványtárolójában.

##<a name="example"></a>Példa

###<a name="request"></a>Kérelem
```
POST /CertificateManagement/api/v1.0/requests/C6BAD97C-F97F-4920-8947-BE980C98C6B5/certificatedata HTTP/1.1

{
    "pfxpassword":"6b75162c-b3b4-4fc1-a653-f12dae98f0c3",
    "certificaterequests":[
        "\r\n"
    ]
}

```
###<a name="response"></a>Válasz
```
HTTP/1.1 200 OK

[
    {
        "TemplateCommonName":"ContosoVirtualSmartCardLimitedRelease-KSP",
        "CertificateId":0,
        "Flags":65536,
        "UseLocalMachineStore":false,
        "Error":null,
        "Pkcs7":"\r\n",
        "Pfx":null,
        "IsExternal":false,
        "isServerGenerated":false,
        "isPfx":false,
        "isPkcs7":true,
        "isKeyHistory":false
    }
]
```       
##<a name="see-also"></a>Lásd még:

- [Microsoft.Clm.Provision.ExecuteOperations.Enroll módszer](https://msdn.microsoft.com/library/microsoft.clm.provision.executeoperations.enroll.aspx)
- [Microsoft.Clm.Shared.Certificates.Certificate osztály](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx)
